...
# this is a pretty hex dumping function
def hexdump(src, length=16):
    print(type(src[0:16]))
    result = []
    digits = 2  # 2 if isinstance(src, bytes) else 4
    for i in range(0, len(src), length):
        s = src[i : i + length]
        hexa = b" ".join([b"%0*X" % (digits, x) for x in s])
        text = b"".join([x.to_bytes() if 0x20 <= x < 0x7F else b"." for x in s])
        result.append("X %-*s %s" % (length * (digits + 1), hexa, text))
    print("\n".join(result))
